...
import { EntityArrayResponseType, OrderItemProductService } from '../service/order-item-product.service';
...
export class OrderItemProductComponent implements OnInit {
  ...
  trackId = (_index: number, item: IOrderItemProduct): string => this.orderItemProductService.getOrderItemProductIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.orderItemProducts = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IOrderItemProduct[] | null): IOrderItemProduct[] {
    const orderItemProductsNew = this.orderItemProducts ?? [];
    if (data) {
      for (const d of data) {
        if (orderItemProductsNew.map(op => op.id).indexOf(d.id) === -1) {
          orderItemProductsNew.push(d);
        }
      }
    }
    return orderItemProductsNew;
  }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.orderItemProductService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
